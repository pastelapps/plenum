  ...
  return h('div', {
    ...
    ...(course.audience_cards || []).map((card) =>
      h('div', {
        ...
        h('div', { style: { display: 'flex', flexShrink: 0, marginTop: 2 } }, getIcon(card.icon, 28, primary)),
        h('div', { style: { display: 'flex', flexDirection: 'column', gap: 4 } },
          h('span', { style: { fontSize: 18, fontFamily: heading, fontWeight: 700, color: '#ffffff' } }, card.title),
          h('span', { style: { fontSize: 15, fontFamily: body, color: '#ffffffbb', lineHeight: 1.4 } }, card.description),
        ),
      ),
    ),
  );
...
function _infoPill(label: string, value: string, fontFamily: string): SatoriNode {
  return h('div', {
    style: { display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center', gap: 4 },
  },
    h('span', { style: { fontSize: 11, fontFamily, color: '#ffffff77', textTransform: 'uppercase', letterSpacing: 1.5 } }, label),
    h('span', { style: { fontSize: 16, fontFamily, fontWeight: 600, color: '#ffffffdd' } }, value),
  );
}
