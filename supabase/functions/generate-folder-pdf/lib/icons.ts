import { h, type SatoriNode } from './types.ts';

// ─── Lucide SVG path data (24x24 viewBox) ────────────────
// Extracted from lucide-static for satori compatibility
...
/**
 * Get Lucide icon as satori-compatible SVG node
 */
export function getIcon(name: string, size: number, color: string): SatoriNode {
  ...
  // Split multi-path icons (space between M commands)
...
/**
 * Get a filled check icon (no stroke, filled circle + check)
 */
